CREATE TABLE `onSiteInventories_ItemHistory` (
...
If the item has special parameters, this field is set to null, so that new items will never be added to this stack.',
  ...
  UNIQUE INDEX UK_onSiteInventories_ItemHistory_id(id),
  INDEX id(id)
)
ENGINE = INNODB,
...
ALTER TABLE `onSiteInventories_ItemHistory` 
...
ALTER TABLE `onSiteInventories_ItemHistory` 
  ADD CONSTRAINT `oihHistoryId` FOREIGN KEY (historyId)
    REFERENCES botInventories_ItemHistory(id);

ALTER TABLE `onSiteInventories_ItemHistory` 
  ADD CONSTRAINT `oihItemHistoryInventoryId` FOREIGN KEY (inventoryId)
